...
type Episode = { id:string; episode_number:number; season_number:number; title:string; summary:string|null; episode_status:string; word_count:number|null; artwork_url:string|null };
...
export default function StudioWorkflow3Client(){
  ...
  return <main className={`w3-shell scheme-${colourScheme}`}>
    ...
    {!loading&&data&&<>
      ...
      {tab==="Episodes"&&<section className="w3-page"><div className="w3-page-title"><div><p>NARRATIVE</p><h1>Episodes</h1></div><span>{data.episodes.length} episodes</span></div><div className="w3-episodes"><aside>{data.episodes.map(e=><button key={e.id} className={episode?.id===e.id?"active":""} onClick={()=>setEpisodeId(e.id)}><span>{String(e.episode_number).padStart(2,"0")}</span><div><strong>{e.title}</strong><small>{pretty(e.episode_status)}</small></div></button>)}</aside><article>{episode?<><p className="w3-kicker">SEASON {episode.season_number} · EPISODE {episode.episode_number}</p><h2>{episode.title}</h2>{episode.artwork_url?<img src={episode.artwork_url} alt=""/>:<div className="w3-art-empty compact">No episode artwork linked</div>}<p className="w3-lead">{episode.summary??"No episode summary loaded."}</p><div className="w3-tags"><span>{pretty(episode.episode_status)}</span><span>{(episode.word_count??0).toLocaleString()} words</span></div></>:<Empty title="No episodes yet">Episode records will appear here.</Empty>}</article></div></section>}
      {tab==="Canon"&&<section className="w3-canon"><div className="w3-canon-nav"><button className={!category?"active":""}>Overview</button>{categories.map(c=><button key={c.slug} className={category===c.slug?"active":""} onClick={()=>{setCategory(c.slug);setRecordId("")}}><span>{icons[c.slug]??"✦"}</span>{aliases[c.slug]??c.name}<small>{c.record_count}</small></button>)}</div><div className="w3-page"><div className="w3-page-title canon"><div><p>THE AUTHORITATIVE WORLD REFERENCE</p><h1>{aliases[category]??pretty(category||"Canon")}</h1></div><span>{records.length} records</span></div>
      ...
      {tab==="Planning"&&<section className="w3-page"><div className="w3-page-title"><div><p>STUDIO WORKSPACE</p><h1>Planning</h1></div></div><Empty title="Planning remains available">No planning data beyond the Season Plan has been invented.</Empty></section>}
